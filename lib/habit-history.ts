...
import type {
  DayCell,
  HistoryStats,
} from "@/components/habits/HabitHistoryClient";
...
/**
 * Builds the 91-day history grid + window stats for a habit. Computed once on
 * the server so the detail page can feed both the stat strip and the grid.
 */
export async function loadHabitHistory(
  habit: Habit,
  timezone: string,
  ...
  return { days, stats, dailyThreshold };
}
